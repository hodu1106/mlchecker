...
(() => {
  ...
  async function startShare(){
    ...
    try{
      ...
      state.stream=stream; els.video.srcObject=stream; await els.video.play(); resizeOverlay(); els.measureBtn.disabled=false; els.stopBtn.disabled=false;
      stream.getVideoTracks()[0].addEventListener('ended', ()=>{stopMeasure(false); if(state.stream) state.stream.getTracks().forEach(t=>t.stop()); state.stream=null; els.video.srcObject=null; state.selection=null; state.displaySelection=null; drawOverlay(); setOCRStatus('화면 공유 종료');});
  ...
  function startMeasure(){if(!state.stream) return alert('먼저 화면 공유를 시작해 주세요.'); if(!state.selection) return alert('경험치 숫자 영역을 먼저 드래그해서 지정해 주세요.');
    saveSettings(); state.running=true; state.startAt=Date.now(); state.baseExp=null; state.lastExp=null; state.history=[]; els.liveBadge.style.display='inline-flex'; setOCRStatus('측정 시작'); updateStats();
  ...
  function stopMeasure(resetRunning=true){if(state.timer) clearInterval(state.timer); state.timer=null; if(resetRunning) state.running=false; els.liveBadge.style.display='none'; updateStats();}
  function resetAll(){stopMeasure(); state.running=false; state.startAt=null; state.baseExp=null; state.lastExp=null; state.history=[]; state.lastRecognizedText=''; state.lastOCRAt=null; setOCRStatus('초기화됨'); updateStats();}
  function drawGraph(){
  ...
  async function openPiP(){
    ...
    const pipWindow=await window.documentPictureInPicture.requestWindow({width:460,height:320}); state.pipWindow=pipWindow;
    pipWindow.document.body.innerHTML=`<style>body{margin:0;padding:16px;background:#0d1429;color:#eef2ff;font-family:Inter,system-ui,sans-serif}.box{display:grid;gap:12px}.big{font-size:40px;font-weight:800;letter-spacing:-.04em}.muted{color:#a7b0d1;font-size:13px}.card{padding:14px 16px;border-radius:18px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10)}</style><div class="box"><div class="card"><div class="muted">경과</div><div class="big" id="pipElapsed">00:00:00</div></div><div class="card"><div class="muted">획득 경험치</div><div class="big" id="pipGained">0</div></div><div class="card"><div class="muted">60분 페이스</div><div class="big" id="pipPace">0</div></div></div>`;
    const sync=()=>{if(!state.pipWindow||state.pipWindow.closed) return; const elapsedMs=state.startAt?Date.now()-state.startAt:0; const gained=(state.baseExp!=null&&state.lastExp!=null)?Math.max(0,state.lastExp-state.baseExp):0; const pace=elapsedMs>0?(gained/elapsedMs)*3600000:0; pipWindow.document.getElementById('pipElapsed').textContent=formatDuration(elapsedMs); pipWindow.document.getElementById('pipGained').textContent=formatNumber(gained); pipWindow.document.getElementById('pipPace').textContent=formatNumber(pace); requestAnimationFrame(sync);}; sync();
  }
  function bindEvents(){
