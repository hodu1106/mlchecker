...
<head>
  ...
  <style>
    :root{--bg:#0b1020;--bg2:#121933;--card:rgba(18,25,51,.72);--line:rgba(255,255,255,.10);--text:#eef2ff;--muted:#a7b0d1;--accent:#7c9cff;--accent2:#4de2c5;--danger:#ff5252;--warning:#ffcf66;--shadow:0 18px 40px rgba(0,0,0,.35);--radius:22px;}
...
(() => {
  ...
  function setOCRStatus(t){els.ocrStatus.textContent=t; syncPiP();}
  function setButtons(){els.measureBtn.disabled=!state.stream||state.running; els.stopBtn.disabled=!state.stream||!state.running; syncPiP();}
  function currentStats(){
    ...
    return {elapsedMs,gained,total,gainedPct,pace,pacePct};
  }
  function updateStats(){
    const s = currentStats();
    els.elapsed.textContent=formatDuration(s.elapsedMs);
    ...
    els.gained.innerHTML=`${formatNumber(s.gained)} <span class="pct">[${formatPct(s.gainedPct)}%]</span>`;
    els.pace.innerHTML=`${formatNumber(s.pace)} <span class="pct">[${formatPct(s.pacePct)}%]</span>`;
    els.logBox.textContent=[`마지막 OCR: ${state.lastOCRAt ? new Date(state.lastOCRAt).toLocaleTimeString('ko-KR') : '-'}`,`마지막 인식 텍스트: ${state.lastRecognizedText || '-'}`,`현재 인식 경험치: ${state.lastExp != null ? formatNumber(state.lastExp) : '-'}`,`초기 기준 경험치: ${state.baseExp != null ? formatNumber(state.baseExp) : '-'}`,`최근 포인트 수: ${state.history.length}`].join('\n');
    drawGraph(); syncPiP();
  }
  ...
  async function startShare(){
    ...
    try{
      ...
      state.stream=stream; els.video.srcObject=stream; await els.video.play(); resizeOverlay(); setButtons();
      stream.getVideoTracks()[0].addEventListener('ended', ()=>{stopMeasure(false); if(state.stream) state.stream.getTracks().forEach(t=>t.stop()); state.stream=null; els.video.srcObject=null; state.selection=null; state.displaySelection=null; drawOverlay(); setOCRStatus('화면 공유 종료'); setButtons();});
  ...
  async function readOCR(){
    ...
    try{
      ...
      if(parsed){
        ...
        const s = currentStats();
        state.history.push({ts:Date.now(), pace:s.pace}); state.history=state.history.filter(p=>Date.now()-p.ts<=30000); setOCRStatus('OCR 정상');
      } else setOCRStatus('숫자 인식 실패');
    }catch(err){console.error(err); setOCRStatus('OCR 오류');}
    finally{state.ocrBusy=false; updateStats();}
  }
  function startMeasure(){
    if(!state.stream) return alert('먼저 화면 공유를 시작해 주세요.');
    if(!state.selection) return alert('경험치 숫자 영역을 먼저 드래그해서 지정해 주세요.');
    saveSettings(); state.running=true; state.startAt=Date.now(); state.baseExp=null; state.lastExp=null; state.history=[]; els.liveBadge.style.display='inline-flex'; setOCRStatus('측정 시작'); setButtons(); updateStats();
  ...
  function stopMeasure(resetRunning=true){if(state.timer) clearInterval(state.timer); state.timer=null; if(resetRunning) state.running=false; els.liveBadge.style.display='none'; setButtons(); updateStats();}
  function resetAll(){stopMeasure(); state.running=false; state.startAt=null; state.baseExp=null; state.lastExp=null; state.history=[]; state.lastRecognizedText=''; state.lastOCRAt=null; setOCRStatus('초기화됨'); setButtons(); updateStats();}
  function drawGraph(){
  ...
  async function copyResultText(){
    const s=currentStats(); const now=new Date().toLocaleString('ko-KR');
    const text=[`[메랜 경험치 측정 결과]`,`시간: ${now}`,`경과: ${formatDuration(s.elapsedMs)}`,`획득 경험치: ${formatNumber(s.gained)}${s.total?` (${formatPct(s.gainedPct)}%)`:''}`,`60분 페이스: ${formatNumber(s.pace)}${s.total?` (${formatPct(s.pacePct)}%)`:''}`,`마지막 OCR: ${state.lastRecognizedText||'-'}`].join('\n');
  ...
  function syncPiP(){
    ...
    setText('pipElapsed', formatDuration(s.elapsedMs));
    setText('pipNextHour', state.startAt ? new Date(state.startAt+3600000).toLocaleTimeString('ko-KR') : '-');
    setText('pipGained', `${formatNumber(s.gained)} [${formatPct(s.gainedPct)}%]`);
    setText('pipPace', `${formatNumber(s.pace)} [${formatPct(s.pacePct)}%] / 60분`);
    const startBtn = doc.getElementById('pipStartPauseBtn');
    if (startBtn) {
      startBtn.textContent = state.running ? '❚❚' : '▶';
      startBtn.setAttribute('data-running', state.running ? '1' : '0');
      startBtn.style.background = state.running ? 'linear-gradient(180deg,#ff6464,#ff4545)' : 'linear-gradient(180deg,#33d16f,#20b85a)';
    }
  }
  ...
  async function openPiP(){
    ...
    const pipWindow=await window.documentPictureInPicture.requestWindow({width:340,height:230});
    ...
    pipWindow.document.body.innerHTML=`<style>
      ...
      .playbtn{width:58px;height:38px;border:none;border-radius:8px;cursor:pointer;font-size:22px;font-weight:900;color:#fff;background:linear-gradient(180deg,#33d16f,#20b85a);box-shadow:0 8px 20px rgba(0,0,0,.28)}
      .time{font-size:32px;font-weight:900;line-height:1}
      .next{margin-top:6px;color:#cfd6e8;font-size:12px;text-align:center}
      .stats{margin-top:8px;text-align:center}
      .main{font-size:18px;font-weight:900;color:#ffca28;line-height:1.25}
      .sub{font-size:15px;font-weight:900;color:#ffca28;line-height:1.25}
      .bottom{margin-top:10px;display:flex;justify-content:center;gap:8px}
      .mini{border:none;border-radius:8px;padding:8px 10px;font-size:12px;font-weight:800;color:#fff;cursor:pointer;background:#202635}
      .mini.warn{background:#6b5322}
    </style>
    <div class="wrap">
      <div class="top">
        <button id="pipStartPauseBtn" class="playbtn">▶</button>
        <div style="flex:1">
          <div id="pipElapsed" class="time">00:00:00</div>
          <div class="next">1시간 되는 시각&nbsp;&nbsp;<span id="pipNextHour">-</span></div>
        </div>
      </div>
      <div class="stats">
  ...
  function bindEvents(){
    window.addEventListener('resize', resizeOverlay); els.video.addEventListener('loadedmetadata', resizeOverlay); els.shareBtn.addEventListener('click', startShare); els.measureBtn.addEventListener('click', startMeasure); els.stopBtn.addEventListener('click', ()=>{state.running=false; stopMeasure(); setOCRStatus('측정 중지');}); els.resetBtn.addEventListener('click', resetAll); els.copyTextBtn.addEventListener('click', copyResultText); els.copyImageBtn.addEventListener('click', ()=>alert('이 버전은 텍스트 복사 중심입니다.')); els.pipBtn.addEventListener('click', openPiP);
  ...
  loadSettings(); setupSelection(); bindEvents(); resizeOverlay(); setButtons(); updateStats();
})();
</script>
</body>
</html>'''
